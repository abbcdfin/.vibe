...
Initialize the current directory as a vibe project. This skill takes one input — the
workspace **mode** — which you read from the user's invocation (skills have no
positional `$1`, so the mode arrives as plain text in the request).
...
mode exactly. It is the single source of truth shared with the Claude Code
`/vibe:init` command — do not re-derive the scaffolding steps here.
